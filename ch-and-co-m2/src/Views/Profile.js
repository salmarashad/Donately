					...
					<div className="h-max-content w-[600px] bg-white rounded-md shadow-lg flex flex-col p-7 gap-4">
								...
								<div className="flex justify-between">
									{userType === "organization" ? (
										<h1 className="font-bold text-2xl">
											Resala{" "}
											<span className="text-farahgray font-normal text-lg">
												(non-profit)
											</span>
										</h1>
									) : (
						...
						<div className="flex flex-col gap-4 items-center">
							<div className="bg-farahgreen-200 rounded-md text-center py-2 w-full text-farahgreen-700">
								<h3>
									...
									{(userType === "doctor" ||
										...
										userType === "organization") && (
										<button
											className="underline cursor-pointer mx-auto italic"
											onClick={downloadCv}
										>
											{userType === "doctor" || userType === "teacher"
												? "view your CV"
												: "view your proof of identity"}
										</button>
									)}
								</h3>
							</div>
							{userType === "donor" && (
								<div>
									<p
										className="underline cursor-pointer w-max mx-auto italic"
							...
							{userType === "teacher" && (
								<div>
									<h3 className="text-center">
										...
										. You can teach{" "}
										<span className="font-semibold italic">
											{teacherData.numCases}
										</span>{" "}
										pro-bono classes per week.
									</h3>
								</div>
							)}
							{userType === "doctor" && (
							...
							{userType === "doctor" && (
								<div>
									<h3 className="text-center -mb-2">
										Clinic location: {doctorData.address}, {doctorData.area},{" "}
										{doctorData.governorate}
									</h3>
								</div>
							)}
							{userType === "organization" && (
								<div>
									<h3 className="text-center -mb-2">
										Organization location: {orgData.address}, {orgData.area},{" "}
										{orgData.governorate}
									</h3>
								</div>
							)}
							{(userType === "doctor" || userType === "organization") && (
								<div className="w-96 overflow-hidden bg-farahgreen-400 rounded-md flex justify-center items-center">
									<Maps
										isStaticMap={true}
										Location={userType === "doctor" ? "Clinic" : "Organization"}
									/>
								</div>
							)}
						</div>
